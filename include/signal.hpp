...
public:
	...
	unsigned short int getSigDataID() const { return sigDataID; }
	FuncType getSigFuncType() const { return sigFuncType; }
	...
	void setSigDataID(const unsigned short int sigDataID) { this->sigDataID = sigDataID; }
	void setSigFuncType(const FuncType sigFuncType) { this->sigFuncType = sigFuncType; }
...
private:
	...
	FuncType sigFuncType = FuncType::Normal;
	// Signal DataID (only CRCsignals has special DataID, others are 0) 
	unsigned short int sigDataID = 0;
};
